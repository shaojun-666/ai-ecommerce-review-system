...
| Module | Tests | Status |
|---|---|---|
| Backend (Flask API + Crawler + Data Pipeline + Cache + Alerts) | 232/232 passed | ✅ Complete |
...
| 10 | Alerts (`/alerts`) | View system alerts: negative sentiment surges, price anomalies, silent products; trigger manual checks |
| 11 | Compare (`/compare`) | Multi-product side-by-side comparison across platforms |
...
| Alerts | Auto-generated by alert detectors |

### Phase 11 — Advanced Features

| Feature | Description |
|---------|-------------|
| **Alert System** | 3 detectors: negative comment surge (1.5x in 7d), price anomalies (±15-20% in 30d), silent products (14d no comments). 24h dedup, severity levels (critical/warning/info) |
| **WebSocket Real-time Push** | Flask-SocketIO with gevent. Dashboard live updates, alert count badges. Polling fallback if WebSocket unavailable |
| **Timed Email Reports** | Celery Beat scheduled: daily summary (9:03) + weekly report (Mon 10:07). SMTP config with styled HTML template |
| **Competitor Discovery** | 4-factor scoring model: tag overlap (40pts) + platform (20pts) + name keywords (30pts) + ID prefix (10pts). API for pair scoring + landscape summary |
| **HTTPS & Production Hardening** | nginx dual-mode config (production SSL + dev HTTP), security headers middleware, log rotation, gzip, static asset caching, hidden file deny |
...
│   ├── app/
│   │   ├── api/          # RESTful 路由 (auth, products, comments, analysis, dashboard, crawl, alerts, competitors)
│   │   ├── crawler/      # 爬虫引擎 (base, anti_bot, adapters/jd/taobao/tmall/pdd)
│   │   ├── models/       # 数据库模型 (User, Product, Comment, CommentAnalysis, AnalysisTask, CrawlTask, Alert, ProductTag)
│   │   ├── services/     # 业务逻辑层 (auth, comment, analysis, sentiment, report, data_pipeline, alert, competitor)
│   │   ├── tasks/        # Celery 异步任务 (含定时邮件报告、警报检测)
│   │   ├── utils/        # 工具函数 (time, response, errors, validators, text_cleaner, cache, email)
│   │   ├── socket_events.py  # WebSocket 实时推送 (SocketIO)
│   │   └── config/       # 配置文件 (Development, Testing, Production)
│   ├── migrations/       # 数据库迁移
│   └── tests/            # 测试
├── frontend/             # Vue 3 前端
│   └── src/
│       ├── views/        # 页面 (Dashboard, Analysis, Products, Comments, Login, Alerts, Compare, CrawlTasks)
│       ├── components/   # 公共组件 (含组件测试)
│       ├── api/          # API 请求
│       ├── store/        # Pinia 状态
│       ├── router/       # 路由配置
│       └── utils/        # 工具 (含 socket.ts WebSocket 客户端)
├── nlp/                  # NLP 模型模块
│   ├── src/
...
| GET | `/api/v1/alerts` | 警报列表 (支持 unread_only/limit 参数) |
| POST | `/api/v1/alerts/<id>/read` | 标记单条警报已读 |
| POST | `/api/v1/alerts/read-all` | 标记全部已读 |
| POST | `/api/v1/alerts/check` | 手动触发警报检测 |
| GET | `/api/v1/alerts/unread-count` | 未读警报数 |
| GET | `/api/v1/competitors` | 竞品发现和分析 |
| GET | `/api/v1/competitors/summary` | 竞品全景统计 |
| WS | `/socket.io/` | WebSocket 实时推送 (dashboard更新/警报通知) |
...
cd backend
pytest tests/ -v
# 23 unit + 44 integration + 59 crawler + 49 data_pipeline + 26 cache + 31 alerts/services
```
